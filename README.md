...
Requisitos: Node 24+, pnpm 10+, Docker.
...
La API queda en `http://localhost:3000/api`. Si el puerto 5432 ya está ocupado en tu
máquina, cambia `POSTGRES_PORT` y `DATABASE_URL` en `.env` antes de `docker compose up`.
